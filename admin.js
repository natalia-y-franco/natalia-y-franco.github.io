...
(function () {
  ...
  function processData(data) {
    guests = (data.invitados || []).map(function (g) {
      ...
      g.enviada = (g.enviada || '').toUpperCase();
      return g;
    });
  ...
  var waActionsSend = document.getElementById('wa-actions-send');
  var waActionsConfirm = document.getElementById('wa-actions-confirm');
  ...
  function waUpdateCard() {
    ...
    // Reset to "send" state (hide confirm buttons)
    waActionsSend.style.display = '';
    waActionsConfirm.style.display = 'none';
  }
  ...
  // Send current guest → opens WhatsApp, then shows confirm/retry
  waSendCurrentBtn.addEventListener('click', function () {
    if (waIndex >= waList.length) return;
    var g = waList[waIndex];
    sendWhatsAppWithImage(g);
    // Show confirm/retry buttons
    waActionsSend.style.display = 'none';
    waActionsConfirm.style.display = '';
  });

  // Confirm: user says they sent it
  document.getElementById('wa-confirm-yes').addEventListener('click', function () {
    waSentCount++;
    waAdvance();
  });

  // Retry: open WhatsApp again for same guest
  document.getElementById('wa-retry').addEventListener('click', function () {
    if (waIndex >= waList.length) return;
    sendWhatsAppWithImage(waList[waIndex]);
  });
  ...
  function showDetail(g) {
    ...
    var enviadaText = g.enviada === 'TRUE' ? 'Sí' : 'No';
  ...
  document.getElementById('btn-export-csv').addEventListener('click', function () {
    ...
    var headers = ['Nombre', 'Acompañante', 'Grupo', 'Teléfono', 'Confirmación', 'Enviada WA', 'Alergias', 'Detalle alergias', 'Link invitación'];
    var rows = guests.map(function (g) {
      ...
      var enviada = g.enviada === 'TRUE' ? 'Sí' : 'No';
      return [g.nombre, g.acompanante, g.grupo, g.telefono, status, enviada, alergia, g.detalleAlergias, buildInvitationUrl(g)];
    });
